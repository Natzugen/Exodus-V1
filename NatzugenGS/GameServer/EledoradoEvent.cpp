		...
		if ( gObj[n].Class == 79 )	// Gold Dercon
		{
			...
			if ( SelMap == -1 )
			{
				...
				case 0:
					...
					AllSendServerMsg(szTemp/*"Golden Dragon Invasion Lorencia !!!"*/);
					break;
				case 2:
					strcat(szTemp, lMsg.Get(MSGGET(7, 210)));
					AllSendServerMsg(szTemp/*"Golden Dragon Invasion Devias !!!"*/);
					break;
				case 3:
					strcat(szTemp, lMsg.Get(MSGGET(7, 211)));
					AllSendServerMsg(szTemp/*"Golden Dragon Invasion Noria!!!"*/);
					break;
				}

				strcat(szTemp, lMsg.Get(MSGGET(4, 134)));
				AllSendServerMsg(szTemp/*"Golden Dragon Invasion !!!"*/);
			}
			//revisar
			this->m_BossGoldDerconMapNumber[count] = MapNumber;
			gObj[n].m_BossGoldDerconMapNumber = MapNumber/*count*/;	// #error change count by MapNumber
			count++;
			SelMap = MapNumber;
		}
	}
