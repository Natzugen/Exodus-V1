...
BOOL CMixSystem::PlusItemLevelChaosMix(LPOBJ lpObj, int mixType)
{
	...
	if ( (rand()%100) <= lpObj->ChaosSuccessRate )
	{
		...
		{
			int m_itemMsgShowDisable = GetPrivateProfileInt("GameServerInfo", "ItemMgsShowHide", 0, gDirPath.GetNewPath("CommonServer.cfg"));

			char Text[256];
			ZeroMemory(Text, sizeof(Text));
			sprintf(Text, "Congratulations! %s made +%d item!", lpObj->Name, Item.m_Level);
			if (m_itemMsgShowDisable == 1) 
			{ 
				AllSendServerMsg(Text); //Show/Hide Mgs before it sends to client.
			}
		}
#endif
		//season4 add-on
