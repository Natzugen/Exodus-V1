...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ((lpMsg->CtlCode & 2) != 2 && lpObj->MapNumber == MAP_INDEX_GM_MAP)
	{
		lpObj->MapNumber = MAP_INDEX_DEVIAS;
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}
	...
	if ( (lpMsg->CtlCode & 0x20 ) == 0x20 )
	{
		...
		LogAddC(2, "(%s)(%s) Set Event Admin", lpObj->AccountID, lpObj->Name);
	...
	if (lpObj->Authority == 16) //Season3 add-on
	{
		gObjAddBuffEffect(lpObj, AT_GAMEMASTER_LOGO, 0, 0, 0, 0, -10);
	}
...
char* gObjGetAccountId(int aIndex)
{
	...
	if (gObj[aIndex].Connected == 2 || g_AutoReconnect == 1 || g_AutoReconnect == 0)
	{
...
#ifdef __NOVUS__
	//if(g_MUHelper.GetUserSlot(lpTargetObj->m_Index) != (int)-1 )//old code
	if(lpTargetObj->m_MUHelperOn)//FIX
	{
...
#ifdef OFFEXP
	SetTextColor(hdc, RGB(255, 10, 33));
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	LogAddTD("[%s][%s] JewelofLife Result [%d,%s,%d,%d,%d,%d]serial:[%u] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
		...
		BYTE(lpObj->pInventory[target].m_Durability),NewOption[0],NewOption[1],NewOption[2],NewOption[3],NewOption[4],NewOption[5],NewOption[6]);
