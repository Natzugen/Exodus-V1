...
int CGMMng::ManagementProc(LPOBJ lpObj, char* szCmd, int aIndex) //00570A00
{
	...
	case COMMAND_RANDOMSETITEM://Identical
		{
			for(int i = 0; i < 20; i++) // 20 random sets ?
			{
	...
	case COMMAND_RESETPK://identical
	{
		...
		break;
	case COMMAND_MAKEPET://identical
	{
		...
		if (getMonsterID == -1) { //Bad Forest Orc
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			//gObjUseSkill.SkillMonsterCall(lpObj->m_Index, 999, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet has been removed");
		}
		if (getMonsterID == 1) { //Totem Golem
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 423, lpObj->X - 1, lpObj->Y + 1);
			pNotice.SendToUser(lpObj->m_Index, "Pet 'Totem Golem' created");
		}
		if (getMonsterID == 2) { //Death King
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 55, lpObj->X - 1, lpObj->Y + 1);
			pNotice.SendToUser(lpObj->m_Index, "Pet 'Death King' created");
		}
		if (getMonsterID == 3) { //Bad Red Golem
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 551, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Red Golem' created");
		}
		if (getMonsterID == 4) { //Bad Witch Queen
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 552, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Witch Queen' created");
		}
		if (getMonsterID == 5) { //Bad Blade Hunter
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 553, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Blade Hunter' created");
		}
		if (getMonsterID == 6) { //Bad Kentauros
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 554, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Kentauros' created");
		}
		if (getMonsterID == 7) { //Bad Genocider
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 556, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Genocider' created");
		}
		if (getMonsterID == 8) { //Bad Sapi-Duo
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 557, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Sapi-Duo' created");
		}
		if (getMonsterID == 9) { //Bad Sapi-Tres
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 558, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Sapi-Tres' created");
		}
		if (getMonsterID == 10) { //Bad Thunder Napin
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 559, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Thunder Napin' created");
		}
		if (getMonsterID == 11) { //Bad Ice Giant
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 563, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Ice Giant' created");
		}
		if (getMonsterID == 12) { //Bad Iron Knight
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 565, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Bad Iron Knight' created");
		}
		if (getMonsterID == 13) { //Gollock
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 572, lpObj->X - 1, lpObj->Y + 1);
				pNotice.SendToUser(lpObj->m_Index, "Pet 'Gollock' created");
		}
		if (getMonsterID == 14) { //Condra
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 575, lpObj->X - 1, lpObj->Y + 1);
			pNotice.SendToUser(lpObj->m_Index, "Pet 'Condra' created");
		}
		if (getMonsterID == 15) { //Narcondra
			GCRecallMonLife(aIndex, 60, 0);
			gObjMonsterCallKill(aIndex);
			gObjUseSkill.SkillMonsterCalls(lpObj->m_Index, 576, lpObj->X - 1, lpObj->Y + 1);
			pNotice.SendToUser(lpObj->m_Index, "Pet 'Narcondra' created");
		}
	}
		break;
	...
	case COMMAND_NOTICEALL://Identical
		...
		break;
	case COMMAND_ggpost://Identical
		{
			TNotice pNotice(0);

			char * dropGGString = this->GetTokenStrings();

			pNotice.SendToAllUser("[%s] %s", lpObj->Name, dropGGString);
		}
		break;
	...
	case COMMAND_SUMMONCHAR:
		...
		break;
	case COMMAND_SKIN:
		this->CmdSkin(lpObj, this->GetTokenString());
		break;
#ifdef __CUSTOMS__
	case COMMAND_ADDSTR:
		{
...
#pragma message("!!!!!!!!!!!!!!!!! ADD PREFIX SETTING !!!!!!!!!!!!!!!!")
				...
				char PostChatPrefix[10]// = [WORLD]
				GetPrivateProfileStringA("GameServerInfo","PostChatPrefix","[WORLD]",PostChatPrefix,10,("../Data/commonserver.cfg"));
