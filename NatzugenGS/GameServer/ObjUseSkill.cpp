...
void CObjUseSkill::UseSkill(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) //004BECB0
{
	...
	//LogAddTD("Casting Skill (UseSkill) [%S]", lpMagic);
...
void CObjUseSkill::UseSkill(int aIndex, int aTargetIndex, CMagicInf * lpMagic) //
{
	...
	//LogAddTD("Casting Skill (UseSkill 2 ) [%S]", lpMagic);
...
void CObjUseSkill::KnightSkillAddLife(int aIndex, int skill_level) //
{
	...
	int iaddlife;
	int iaddLifepower = 0; // g_SkillAdditionInfo.m_SwellLifeEffectStart + (lpObj->Vitality + lpObj->AddVitality) / g_SkillAdditionInfo.m_SwellLifeEffectDiv1 + (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_SwellLifeEffectDiv2 + viewpercent;
	
	if( g_SkillAdditionInfo.m_SwellLifeEffectMax > 0 )
	{
	...
	int iLifeTime = 0; // g_SkillAdditionInfo.m_SwellLifeTimeStart + (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_SwellLifeTimeDiv;
	
	if( g_SkillAdditionInfo.m_SwellLifeTimeMax > 0 )
	{
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, int skill_level) //004C0560
{
	...
	int WizardSkillDefense = 0; //g_SkillAdditionInfo.m_SoulBarrierDefenseStart + ((lpObj->Dexterity + lpObj->AddDexterity) / g_SkillAdditionInfo.m_SoulBarrierDefenseDiv1) + ((lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_SoulBarrierDefenseDiv2);
	// ----
	if( g_SkillAdditionInfo.m_SoulBarrierDefenseMax > 0 )
	{
		WizardSkillDefense = g_SkillAdditionInfo.m_SoulBarrierDefenseMax;
	}
	else {
		WizardSkillDefense = 60;
	}
	// ----
	int WizardSkillDefenseTime = 0; // g_SkillAdditionInfo.m_SoulBarrierTimeStart + (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_SoulBarrierTimeDiv;
	// ----
	if( g_SkillAdditionInfo.m_SoulBarrierTimeMax > 0 )
	{
		WizardSkillDefenseTime = g_SkillAdditionInfo.m_SoulBarrierTimeMax;
	}
	else{
		WizardSkillDefenseTime = 60;
	}
	// ----
...
void CObjUseSkill::SkillDefense(int aIndex, int aTargetIndex, int skill_level) //
{
	...
	float skill_defense = 0; //float(skill_level + 1)*g_SkillAdditionInfo.m_ElfDefenseEffectFact + (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_ElfDefenseEffectDiv;

	if( g_SkillAdditionInfo.m_ElfDefenseEffectMax > 0 )
	{
...
void CObjUseSkill::SkillAttack(int aIndex, int aTargetIndex, int skill_level) //
{
	...
	float skill_attack = 0; // float((skill_level + 1) * g_SkillAdditionInfo.m_ElfAttackEffectFact) + (lpObj->Energy + lpObj->AddEnergy) / (float)g_SkillAdditionInfo.m_ElfAttackEffectDiv;
	int BuffTime		= g_SkillAdditionInfo.m_ElfAttackTime;

	if( g_SkillAdditionInfo.m_ElfAttackEffectMax > 0 )
	{
...
int CObjUseSkill::SkillMonsterCalls(int aIndex, int MonsterType, int x, int y) //identical gs-cs 56
{
	...
	if (result >= 0)
	{
		...
		gObj[result].m_Attribute = 100; // 100
		...
		//Increase Summoned Power
		gObj[result].Level = 250;
		gObj[result].m_Defense += 1000;
		gObj[result].m_AttackDamageMax += 26000;
		gObj[result].m_AttackDamageMin += 16000;
		// end

		LogAddTD("Monster Created");
...
void CObjUseSkill::SkillPowerSlash(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE targetangle, int aTargetIndex) //
{
	...
	//LogAdd("MAGIC: %s | AttackSpeed: %d | MagicSpeed: %d", lpMagic, gObj[aIndex].m_AttackSpeed, gObj[aIndex].m_MagicSpeed);
...
void CObjUseSkill::SkillAddCriticalDamage(int aIndex, int skill_level) //004C4920
{
	...
	int addcriticaldamagevalue = 0; // (lpObj->Leadership + lpObj->AddLeadership) / g_SkillAdditionInfo.m_CriticalDamageEffectDiv1 + (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_CriticalDamageEffectDiv2;
	
	if( g_SkillAdditionInfo.m_CriticalDamageEffectMax > 0 )
	{
		addcriticaldamagevalue = g_SkillAdditionInfo.m_CriticalDamageEffectMax;
	}
	else {
		addcriticaldamagevalue = 7000;
	}

	int SkillTime = 0; // (lpObj->Energy + lpObj->AddEnergy) / g_SkillAdditionInfo.m_CriticalDamageTimeDiv + g_SkillAdditionInfo.m_CriticalDamageTimeStart;

	if( g_SkillAdditionInfo.m_CriticalDamageTimeMax > 0 )
	{
...
int CObjUseSkill::SkillDamageReflection(int aIndex,int aTargetIndex,CMagicInf * lpMagic) //004C8CF0
{
	...
	{
		int iValue = 0; // g_SkillAdditionInfo.m_ReflectEffectStart + (lpObj->Energy / g_SkillAdditionInfo.m_ReflectEffectDiv);
		
		if( g_SkillAdditionInfo.m_ReflectEffectMax > 0 )
		{
		...
		int iTimer = 0; // g_SkillAdditionInfo.m_ReflectTimeStart + (lpObj->Energy / g_SkillAdditionInfo.m_ReflectTimeDiv);
		
		if( g_SkillAdditionInfo.m_ReflectTimeMax > 0 )
		{
...
BOOL CObjUseSkill::SkillBerserker(int aIndex, CMagicInf* lpMagic) //004F74C0
{
	...
	int iDuration = 0; // lpObj->Energy / g_SkillAdditionInfo.m_BerserkerTimeDiv + g_SkillAdditionInfo.m_BerserkerTimeStart;
	
	if( g_SkillAdditionInfo.m_BerserkerTimeMax > 0 )
	{
...
BOOL CObjUseSkill::SkillExpansionWizardry(int aIndex, int aTargetIndex, CMagicInf *lpMagic) //004F8CF0
{
	...
	int iWizardryDamage = 0; // (float)(lpObj->Energy / g_SkillAdditionInfo.m_WizardryEnchantDamageDiv) * (float)g_SkillAdditionInfo.m_WizardryEnchantDamageFact;

	if( g_SkillAdditionInfo.m_WizardryEnchantDamageMax > 0 )
	{
...
int CObjUseSkill::SkillMonkBuff(int aIndex, CMagicInf * lpMagic)
{
	...
	// ----
	int nEffectValue = 0;// g_SkillAdditionInfo.m_IgnoreDefenseEffectStart + (lpObj->Energy + lpObj->AddEnergy - g_SkillAdditionInfo.m_IgnoreDefenseEffectSub) / (float)g_SkillAdditionInfo.m_IgnoreDefenseEffectDiv;
	
	if( g_SkillAdditionInfo.m_IgnoreDefenseEffectMax > 0 )
	{
	...
	int Time = 0;// lpObj->Energy / g_SkillAdditionInfo.m_IgnoreDefenseTimeDiv + g_SkillAdditionInfo.m_IgnoreDefenseTimeStart;
	
	if( g_SkillAdditionInfo.m_IgnoreDefenseTimeMax > 0 )
	{
...
int CObjUseSkill::SkillMonkBuffApplyParty(int aIndex, CMagicInf * lpMagic)
{
	...
	{
		nEffectValue = 0;// g_SkillAdditionInfo.m_IncreaseHealthEffectStart + (lpObj->Energy + lpObj->AddEnergy - g_SkillAdditionInfo.m_IncreaseHealthEffectSub) / g_SkillAdditionInfo.m_IncreaseHealthEffectDiv;

		if( g_SkillAdditionInfo.m_IncreaseHealthEffectMax > 0 )
		{
		...
		Time = 0;// g_SkillAdditionInfo.m_IncreaseHealthTimeStart + lpObj->Energy / g_SkillAdditionInfo.m_IncreaseHealthTimeDiv;
		
		if( g_SkillAdditionInfo.m_IncreaseHealthTimeMax > 0 )
		{
	...
	{
		nEffectValue = 0;// g_SkillAdditionInfo.m_IncreaseBlockEffectStart + (lpObj->Energy + lpObj->AddEnergy - g_SkillAdditionInfo.m_IncreaseBlockEffectSub) / g_SkillAdditionInfo.m_IncreaseBlockEffectDiv;

		if( g_SkillAdditionInfo.m_IncreaseBlockEffectMax > 0 )
		{
		...
		Time = 0;// g_SkillAdditionInfo.m_IncreaseBlockTimeStart + lpObj->Energy / g_SkillAdditionInfo.m_IncreaseBlockTimeDiv;
		
		if( g_SkillAdditionInfo.m_IncreaseBlockTimeMax > 0 )
		{
