...
void ResetSystem::ReadGroupData(char * File)
{
	...
	while(true)
	{
		...
		if( Token == END )
		{
			//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: Token == END!");
			break;
		}
		// ----
		if( Token != NUMBER )
		{
			//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: Token != number");
			continue;
		}
		// ----
		int GroupID = g_ScriptEncode.GetNumber();
		//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: GroupID: %d", GroupID);
		// ----
		g_ScriptEncode.GetToken();
		this->m_GroupData[GroupID].MinReset		= g_ScriptEncode.GetNumber();
		//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: MinReset: %d", this->m_GroupData[GroupID].MinReset);  
		// ----
		g_ScriptEncode.GetToken();
		this->m_GroupData[GroupID].MaxReset		= g_ScriptEncode.GetNumber();
		//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: MaxReset: %d", this->m_GroupData[GroupID].MaxReset);
		// ----
		g_ScriptEncode.GetToken();
		this->m_GroupData[GroupID].ReqLevel		= g_ScriptEncode.GetNumber();
		//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: ReqLevel: %d", g_ScriptEncode.GetNumber());
		// ----
		g_ScriptEncode.GetToken();
		this->m_GroupData[GroupID].ReqMoney		= g_ScriptEncode.GetNumber();
		//Log.ConsoleOutPut(1, c_Red, t_Default, "ResetSystem Debug: ReqMoney: %d", g_ScriptEncode.GetNumber());
		// ----
		g_ScriptEncode.GetToken();
