...
/*
void User::DrawMonsterLife(TARGET_DATA td)
{
VAngle Angle;
int PosX, PosY, LifeProgress;
Angle.X = td.Obj->m_Model.VecPosX;
Angle.Y = td.Obj->m_Model.VecPosY;
Angle.Z = td.Obj->m_Model.VecPosZ + td.Obj->m_Model.Unknown216.Z + 100.0;
// ----
float BarWidth = (64.5f / 100.0f) *td.TargetLifePercent;
float LifeBarWidth = 65.0f;
// ----
pGetPosFromAngle(&Angle, &PosX, &PosY);
PosX -= (int)floor(LifeBarWidth / (double)2.0);
// ----
if (td.TargetLifePercent <= 0.5f || !td.Obj->IsLiveObject)
{
return;
}
pDrawGUI(0x7B3F, (float)PosX - 1.2, (float)PosY - 1.5, 66.95, 6);
pDrawGUI(0x7B40, (float)PosX, (float)PosY, BarWidth, 3);
pDrawText(pTextThis(), PosX - 1, PosY - 11, " ", 67, 0, (LPINT)0, 0);
pDrawText(pTextThis(), PosX + 0.5*(66.95 - (4.35*float(strlen(td.Obj->Name)))), PosY - 11, td.Obj->Name, (4.35*float(strlen(td.Obj->Name))), 0, (LPINT)0, 0);
//====
if ((int)(td.TargetLifePercent / 10) > 10)
{
LifeProgress = 10;
}
else
{
LifeProgress = (int)(td.TargetLifePercent / 10);
}
pGLSwitch();
glColor3f(1.0, 1.0, 1.0);
